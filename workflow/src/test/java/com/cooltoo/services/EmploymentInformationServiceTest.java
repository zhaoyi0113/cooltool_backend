...
public class EmploymentInformationServiceTest extends AbstractCooltooTest {
    ...
    public void testCountEmploymentInfoByStatus() {
        long count = service.countEmploymentInfoByStatus(statusAll);
        Assert.assertEquals(9, count);
        count = service.countEmploymentInfoByStatus(statusDis);
        Assert.assertEquals(3, count);
        count = service.countEmploymentInfoByStatus(statusEnb);
        Assert.assertEquals(5, count);
        count = service.countEmploymentInfoByStatus(statusDel);
    ...
    public void testGetEmploymentInfoByStatus() {
        List<EmploymentInformationBean> page0 = service.getEmploymentInfoByStatus(statusAll, 0, 5);
        Assert.assertNotNull(page0);
        Assert.assertEquals(5, page0.size());
        page0 = service.getEmploymentInfoByStatus(statusAll, 1, 5);
        Assert.assertNotNull(page0);
        Assert.assertEquals(4, page0.size());

        page0 = service.getEmploymentInfoByStatus(statusEnb, 0, 3);
        Assert.assertNotNull(page0);
        Assert.assertEquals(3, page0.size());
        page0 = service.getEmploymentInfoByStatus(statusEnb, 1, 3);
        Assert.assertNotNull(page0);
        Assert.assertEquals(2, page0.size());

        page0 = service.getEmploymentInfoByStatus(statusDis, 0, 2);
        Assert.assertNotNull(page0);
        Assert.assertEquals(2, page0.size());
        page0 = service.getEmploymentInfoByStatus(statusDis, 1, 2);
        Assert.assertNotNull(page0);
        Assert.assertEquals(1, page0.size());

        page0 = service.getEmploymentInfoByStatus(statusDel, 0, 2);
        Assert.assertNotNull(page0);
        Assert.assertEquals(1, page0.size());


        page0 = service.getEmploymentInfoByStatus(statusAll);
        Assert.assertNotNull(page0);
        Assert.assertEquals(9, page0.size());

        page0 = service.getEmploymentInfoByStatus(statusDis);
        Assert.assertNotNull(page0);
        Assert.assertEquals(3, page0.size());

        page0 = service.getEmploymentInfoByStatus(statusEnb);
        Assert.assertNotNull(page0);
        Assert.assertEquals(5, page0.size());

        page0 = service.getEmploymentInfoByStatus(statusDel);
        Assert.assertNotNull(page0);
        Assert.assertEquals(1, page0.size());
    }
    ...
    public void testCreateEmploymentInfo() {
        ...
        EmploymentInformationBean bean = service.createEmploymentInfo(title, url, -1);
    ...
    public void testUpdateEmploymentInfoBasicInfo() {
        ...
        EmploymentInformationBean bean1 = service.updateEmploymentInfo(id, title, url, grade, status, null, image);
